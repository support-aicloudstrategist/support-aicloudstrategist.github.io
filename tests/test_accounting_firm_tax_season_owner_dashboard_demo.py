...
SLUG = "global-accounting-firm-tax-season-owner-dashboard-demo"
...
def test_page_is_indexable_and_canonical() -> None:
    ...
    assert "Accounting Firm Tax Season Owner Dashboard Demo" in source
...
def test_structured_data_and_dataset_are_parseable() -> None:
    ...
    assert "accounting firm missed calls" in as_text
...
def test_synthetic_csv_and_svg_assets_exist() -> None:
    ...
    assert len(rows) >= 7
    assert {"source", "service_line", "client_stage", "blocker", "portal_state", "owner", "age_band", "adviser_question", "ai_boundary", "decision_status"} <= set(rows[0])
    assert any("Missed phone call" in row["source"] for row in rows)
    assert SVG.is_file()
    svg = SVG.read_text(encoding="utf-8")
    assert "Synthetic accounting firm tax season owner dashboard demo" in svg
...
def test_truth_boundary_prevents_fake_proof() -> None:
    ...
    for phrase in [
        ...
        "not production accounting firm data",
        "not taxpayer data",
        "not client data",
        "not tax advice",
...
def test_discovery_links_are_wired() -> None:
    ...
    checklist = (ROOT / "resources" / "global-accounting-firm-tax-season-client-intake-follow-up-checklist" / "index.html").read_text(encoding="utf-8")
...
def test_related_cluster_links_exist() -> None:
    ...
    for target in [
        ...
        "/growth-control-os/",
        "/case-studies/",
        f"/free-business-review/?package={SLUG}",
    ]:
