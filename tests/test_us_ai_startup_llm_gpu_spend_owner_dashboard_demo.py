...
SLUG = "us-ai-startup-llm-gpu-spend-owner-dashboard-demo"
...
def test_page_is_indexable_and_canonical() -> None:
    ...
    assert "LLM/GPU spend owner dashboard demo" in source
...
def test_structured_data_and_dataset_are_parseable() -> None:
    ...
    assert "US AI startup FinOps" in as_text
...
def test_synthetic_csv_and_svg_assets_exist() -> None:
    ...
    assert len(rows) >= 5
    assert {"source", "product_area", "owner", "decision_status", "approval_boundary"} <= set(rows[0])
    assert any("LLM API" in row["source"] for row in rows)
    assert SVG.is_file()
    svg = SVG.read_text(encoding="utf-8")
    assert "Synthetic US AI startup LLM and GPU spend owner dashboard demo" in svg
...
def test_discovery_links_are_wired() -> None:
    ...
    for source in [resource_index, llms, sitemap_builder, sitemap]:
...
def test_existing_startup_cluster_links_back_to_demo() -> None:
    for relative in [
        "resources/us-ai-startup-ai-spend-board-review-checklist/index.html",
        "resources/us-ai-startup-llm-gpu-finops-diagnostic-package/index.html",
        "resources/us-ai-startup-llm-gpu-finops-vs-cloud-cost-tools-comparison/index.html",
    ]:
        assert f"/resources/{SLUG}/" in (ROOT / relative).read_text(encoding="utf-8")
