...
def test_pricing_surfaces_healthtech_questionnaire_owner_handoff_offer():
    ...
    assert "Eighteen concrete first offers" in html
    ...
    assert len(re.findall(r'<article class="card"><h3>', section)) == 18


def test_pricing_json_ld_includes_18_fixed_scope_diagnostics():
    ...
    assert len(item_list["itemListElement"]) == 18
