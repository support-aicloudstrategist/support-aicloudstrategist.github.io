...
def test_pricing_surfaces_healthtech_questionnaire_owner_handoff_offer():
    ...
    assert "Sixteen concrete first offers" in html
    ...
    assert len(re.findall(r'<article class="card"><h3>', section)) == 16
...
def test_pricing_json_ld_includes_15_fixed_scope_diagnostics():
    ...
    assert item_list["numberOfItems"] == 16
    assert len(item_list["itemListElement"]) == 16
    last = item_list["itemListElement"][-1]
    assert last["position"] == 16
