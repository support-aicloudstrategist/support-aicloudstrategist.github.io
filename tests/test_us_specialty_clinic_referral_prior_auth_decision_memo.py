...
class UsSpecialtyClinicReferralPriorAuthDecisionMemoTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_page_is_indexable_canonical_and_single_h1(self):
        ...
        self.assertIn("US Specialty Clinic Referral + Prior Auth Decision Memo", self.html)

    def test_buyer_language_and_top_five_context_exist(self):
        for phrase in [
            "US specialty clinic referral leakage",
            "prior authorization delays",
            "patient access owner queue",
            "abandoned calls specialty clinic",
            "eligibility check backlog",
            "AI receptionist HIPAA boundary",
            "referral triage follow-up",
            "payer status owner handoff",
    ...
    def test_csv_has_expected_decision_fields(self):
    ...
    def test_truth_boundaries_prevent_fake_proof(self):
        for phrase in [
            "synthetic template and readiness asset",
    ...
    def test_json_ld_and_discovery_wiring_are_valid(self):
        ...
        self.assertIn(f"US specialty clinic referral prior authorization decision memo: {URL}", self.llms)
