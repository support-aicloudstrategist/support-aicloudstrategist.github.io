...
SLUG = "us-medical-group-no-show-recovery-owner-dashboard-checklist"
...
class UsMedicalGroupNoShowRecoveryOwnerDashboardChecklistTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_page_is_indexable_canonical_and_single_h1(self):
        ...
        self.assertIn("US Medical Group No-Show Recovery Owner Dashboard Checklist", self.html)
    ...
    def test_csv_has_expected_fields_and_boundaries(self):
        ...
        for row in self.rows:
            ...
            self.assertTrue(row["human_review_trigger"])
            self.assertIn("Do not claim", row["unsafe_claim_boundary"])
    ...
    def test_json_ld_and_discovery_wiring_are_valid(self):
        ...
        self.assertIn(f"no-show recovery owner dashboard checklist: {URL}", self.llms)
