...
ROOT=Path('/home/OpenClaw/.openclaw/workspace/reports/sparsh-whatsapp-board-demo-20260519/videos-v8')
...
def header_footer(d, package):
    ...
    d.rounded_rectangle((46,28,94,76), radius=15, fill=NAVY)
    d.text((60,40),'AI',font=FONT['smallb'],fill=GREEN)
    d.text((118,31),'AICloudStrategist',font=FONT['brand'],fill=TEXT)
    d.text((120,61),'Cloud, automation and AI workflows for growing businesses',font=FONT['micro'],fill=MUTED)

    # Package badge with ample width
    d.rounded_rectangle((930,30,1234,76), radius=23, fill=(147,215,190))
    tw=d.textbbox((0,0),package,font=FONT['smallb'])[2]
    d.text((930+(304-tw)//2,44),package,font=FONT['smallb'],fill=NAVY)

    # Premium contact footer: clean rows with full resources and breathing room
    d.line((46,624,1234,624), fill=BORDER, width=2)
    d.rounded_rectangle((46,636,1234,708), radius=18, fill=(255,255,255), outline=BORDER, width=1)
    d.text((72,648),'Website',font=FONT['micro'],fill=MUTED)
    d.text((72,670),'aicloudstrategist.com',font=FONT['small'],fill=TEXT)
    d.text((365,648),'WhatsApp',font=FONT['micro'],fill=MUTED)
    d.text((365,670),'+91 87963 02608',font=FONT['small'],fill=TEXT)
    d.text((620,648),'Voice Line',font=FONT['micro'],fill=MUTED)
    d.text((620,670),'+91 80654 80898',font=FONT['small'],fill=TEXT)
    d.text((860,648),'Email',font=FONT['micro'],fill=MUTED)
    d.text((860,670),'contact@aicloudstrategist.com',font=FONT['micro'],fill=TEXT)
    d.text((860,690),'Support: support@aicloudstrategist.com',font=FONT['micro'],fill=MUTED)
...
def title_slide(package, title, subtitle):
    ...
    draw_multiline(d,'This short demo explains the clinic problem, the solution we build, what components are included, and the benefit for the customer.',(105,448),FONT['body'],TEXT,1000,8)
    return im
...
def closing_slide(package, title, body):
    im=bg(); d=ImageDraw.Draw(im); header_footer(d, package)
    d.text((70,140),title.replace(' and ', ' & ').title(),font=FONT['h1'],fill=TEXT)
    draw_multiline(d,body,(75,215),FONT['body'],(51,65,95),1050,10)
    rounded(d,(90,400,1190,604),NAVY,26,outline=NAVY)
    d.text((130,430),'Thank you',font=FONT['h2'],fill=GREEN)
    draw_multiline(d,'AICloudStrategist can show the demo first, understand the clinic process, and then suggest the smallest practical pilot. No patient medical records are needed for the demo.',(130,482),FONT['body'],WHITE,980,8)
    return im
...
def render_video(pkg_key, ddir, images, audios, durs):
    ...
    out=ddir/f'aics-{pkg_key}-clinic-demo-v8.mp4'
