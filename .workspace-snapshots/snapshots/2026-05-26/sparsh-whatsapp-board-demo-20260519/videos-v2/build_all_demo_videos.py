...
ROOT=Path('/home/OpenClaw/.openclaw/workspace/reports/sparsh-whatsapp-board-demo-20260519/videos-v2')
...
FONT={
 'brand': ImageFont.truetype(str(fd/'DejaVuSans-Bold.ttf'), 24),
 ...
 'tiny': ImageFont.truetype(str(fd/'DejaVuSans.ttf'), 15),
}
...
def header_footer(d, package):
    # Brand capsule wide enough; avoids text overflow
    rounded(d,(46,32,390,80),NAVY,24,outline=NAVY)
    d.text((72,46),'AICloudStrategist',font=FONT['brand'],fill=WHITE)
    d.rounded_rectangle((1010,34,1234,78), radius=22, fill=GREEN)
    d.text((1034,48),package,font=FONT['smallb'],fill=NAVY)
    # footer contact as wrapped two lines if needed
    y=662
    d.line((46,646,1234,646), fill=BORDER, width=2)
    d.text((46,y),'Contact resources',font=FONT['smallb'],fill=TEXT)
    draw_multiline(d, CONTACT, (210,y), FONT['tiny'], MUTED, 1010, 4)

def title_slide(package, title, subtitle):
    im=bg(); d=ImageDraw.Draw(im); header_footer(d, package)
    d.text((70,160), title, font=FONT['h1'], fill=TEXT)
    draw_multiline(d, subtitle, (72,230), FONT['body'], (51,65,95), 860, 10)
    rounded(d,(72,390,1180,525),WHITE,26)
    d.text((105,420),'Welcome',font=FONT['h2'],fill=BLUE)
    draw_multiline(d,'This short demo explains the clinic problem, the solution we build, what components are included, and the benefit for the customer.',(105,466),FONT['body'],TEXT,1000,8)
    return im
...
def render_video(pkg_key, ddir, images, audios, durs):
    ...
    out=ddir/f'aics-{pkg_key}-clinic-demo-v2.mp4'
