...
DATE = '2026-09-01'
SLOT = 'morning'
SLUG = 'ai-workflow-boundary-card'
TITLE = 'The AI Workflow Boundary Card'
HOOK = 'A safe educational one-page card for deciding what AI can draft, what humans must approve, and what should never be automated blindly.'
...
def wrap(text, width=39, max_lines=5):
...
zone_colors = {'Green':('#dcfce7','#166534'), 'Amber':('#fef3c7','#92400e'), 'Red':('#fee2e2','#991b1b')}
zone_svg=[]
for i,(zone,label,text) in enumerate(zones):
    x = 86 + i*366
    fill, ink = zone_colors[zone]
    lines=''.join(f"<tspan x='{x+30}' dy='{0 if n==0 else 24}'>{ln}</tspan>" for n,ln in enumerate(wrap(text, 34, 5)))
    zone_svg.append(f"""
    <g filter='url(#shadow)'>
      <rect x='{x}' y='300' width='318' height='306' rx='34' fill='white' stroke='{ink}' stroke-opacity='.28' stroke-width='3'/>
      <rect x='{x+24}' y='326' width='270' height='76' rx='24' fill='{fill}'/>
      <text x='{x+44}' y='358' font-family='Inter,Arial,sans-serif' font-size='17' font-weight='950' fill='{ink}' letter-spacing='2'>{zone.upper()} ZONE</text>
      <text x='{x+44}' y='390' font-family='Inter,Arial,sans-serif' font-size='26' font-weight='950' fill='#0f172a'>{esc(label)}</text>
      <text x='{x+30}' y='462' font-family='Inter,Arial,sans-serif' font-size='20' fill='#334155'>{lines}</text>
    </g>""")

step_svg=[]
for i,step in enumerate(review_steps):
    y=684+i*54
    lines=''.join(f"<tspan x='184' dy='{0 if n==0 else 18}'>{ln}</tspan>" for n,ln in enumerate(wrap(step, 76, 2)))
    step_svg.append(f"""
    <g>
      <circle cx='124' cy='{y}' r='21' fill='#dbeafe' stroke='#60a5fa'/>
      <text x='124' y='{y+7}' text-anchor='middle' font-family='Inter,Arial,sans-serif' font-size='17' font-weight='950' fill='#1d4ed8'>{i+1}</text>
      <text x='184' y='{y+6}' font-family='Inter,Arial,sans-serif' font-size='18' fill='#334155'>{lines}</text>
    </g>""")

svg=f"""<svg xmlns='http://www.w3.org/2000/svg' width='1200' height='960' viewBox='0 0 1200 960'>
  <defs>
    <linearGradient id='hero' x1='0' x2='1' y1='0' y2='1'><stop stop-color='#0f172a'/><stop offset='.55' stop-color='#2563eb'/><stop offset='1' stop-color='#0f766e'/></linearGradient>
    <filter id='shadow' x='-10%' y='-20%' width='120%' height='150%'><feDropShadow dx='0' dy='16' stdDeviation='12' flood-color='#0f172a' flood-opacity='.13'/></filter>
  </defs>
  <rect width='1200' height='960' fill='#f7fbff'/>
  <rect x='42' y='38' width='1116' height='884' rx='44' fill='white' stroke='#bfdbfe' stroke-width='3'/>
  <rect x='74' y='72' width='1052' height='170' rx='36' fill='url(#hero)'/>
  ...
  <text x='108' y='175' font-family='Inter,Arial,sans-serif' font-size='50' font-weight='950' fill='white'>{esc(TITLE)}</text>
  <text x='108' y='216' font-family='Inter,Arial,sans-serif' font-size='20' fill='#dcfce7'>{esc(HOOK)}</text>
  <text x='88' y='276' font-family='Inter,Arial,sans-serif' font-size='24' font-weight='950' fill='#0f172a'>Three-zone AI operating rule</text>
  {''.join(zone_svg)}
  <rect x='86' y='640' width='1028' height='230' rx='34' fill='#f8fafc' stroke='#cbd5e1'/>
  <text x='108' y='646' transform='translate(0 0)' font-family='Inter,Arial,sans-serif' font-size='0'></text>
  <text x='108' y='650' font-family='Inter,Arial,sans-serif' font-size='22' font-weight='950' fill='#0f172a'>Four checks before AI work becomes an action</text>
  {''.join(step_svg)}
  <text x='108' y='905' font-family='Inter,Arial,sans-serif' font-size='14' fill='#64748b'>{esc(BOUNDARY)}</text>
</svg>"""
...
zones_html=''.join(f"<li><strong>{esc(zone)} — {esc(label)}:</strong> {esc(text)}</li>" for zone,label,text in zones)
steps_html=''.join(f"<li>{esc(step)}</li>" for step in review_steps)
zones_md=''.join(f"- **{zone} — {label}:** {text}\n" for zone,label,text in zones)
...
(info_dir / 'analysis.md').write_text(f"# Analysis — {TITLE}\n\n- Topic: safe AI workflow boundaries.\n- Data type: educational checklist with three risk zones and four review checks.\n- Layout: bento-grid / comparison cards.\n- Style: corporate Memphis with clean trust colors and modular zones.\n- Audience: business operators using AI assistants in daily workflows.\n- Language: en.\n- Safety: educational only; no client, legal, compliance, medical, savings, ranking, or guaranteed claims.\n", encoding='utf-8')
(info_dir / 'structured-content.md').write_text(f"# Structured content — {TITLE}\n\n## Learning objective\nHelp operators classify AI-assisted work into draft-safe, approval-required, and never-blindly-automated zones before outputs become actions.\n\n## Three-zone rule\n{zones_md}\n## Review checks\n{steps_md}\n## Boundary\n{BOUNDARY}\n", encoding='utf-8')
(info_dir / 'prompts' / 'infographic.md').write_text(f"Create a clean corporate Memphis bento-grid infographic titled '{TITLE}' using the three risk-zone cards and four review checks from structured-content.md. Use blue/teal trust colors with green/amber/red cards, numbered review markers, clear hierarchy, and safe educational language. Aspect 16:9-ish wide worksheet.\n", encoding='utf-8')
# homepage latest card and link
home=REPO / 'index.html'
home_text=home.read_text(encoding='utf-8')
new_article=f'''          <article class="ea-evidence-item">
            <div class="ea-mini-art ea-architecture-art" aria-hidden="true"><span>Draft</span><i></i><span>Approve</span><i></i><span>Pause</span></div>
...
home_text=re.sub(r'/publications/\d{4}-\d{2}-\d{2}/[^"<>]+\.html">Review the [^<]+', f'/publications/{DATE}/{SLUG}.html">Review the AI workflow boundary card', home_text, count=1)
...
if llms.exists():
    ...
    line=f'- [{TITLE}]({URL}) — safe educational AI workflow boundary card with infographic.\n'
    if URL not in txt:
