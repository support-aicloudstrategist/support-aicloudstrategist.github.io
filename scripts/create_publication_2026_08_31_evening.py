...
DATE = '2026-08-31'
SLOT = 'evening'
SLUG = 'ai-customer-promise-risk-pause'
TITLE = 'The AI Customer Promise Risk Pause'
HOOK = 'A safe educational checklist for reviewing AI-assisted customer replies before they create promises, expose private details, or hide uncertainty.'
...
colors=['#fee2e2','#dbeafe','#ede9fe','#fef3c7','#ccfbf1','#dcfce7']
card_svg=[]
for i, ((label,text), x, y) in enumerate([(checks[0],72,294),(checks[1],432,294),(checks[2],792,294),(checks[3],72,536),(checks[4],432,536),(checks[5],792,536)]):
    lines=''.join(f"<tspan x='{x+28}' dy='{0 if n==0 else 20}'>{ln}</tspan>" for n,ln in enumerate(wrap(text, 34, 4)))
    card_svg.append(f"""
    <g filter='url(#shadow)'>
      <rect x='{x}' y='{y}' width='320' height='188' rx='30' fill='white' stroke='#d9e2ef'/>
      <rect x='{x+24}' y='{y+24}' width='54' height='54' rx='18' fill='{colors[i]}'/>
      <path d='M{x+42} {y+51} l9 9 l19 -24' fill='none' stroke='#0f172a' stroke-width='7' stroke-linecap='round' stroke-linejoin='round'/>
      <text x='{x+92}' y='{y+57}' font-family='Inter,Arial,sans-serif' font-size='23' font-weight='950' fill='#0f172a'>{esc(label)}</text>
      <text x='{x+28}' y='{y+116}' font-family='Inter,Arial,sans-serif' font-size='16.5' fill='#475569'>{lines}</text>
    </g>""")

step_svg=[]
for i, step in enumerate(usage_steps):
    x=108+i*330
    lines=''.join(f"<tspan x='{x+22}' dy='{0 if n==0 else 18}'>{ln}</tspan>" for n,ln in enumerate(wrap(step, 33, 3)))
    step_svg.append(f"""
    <g>
      <rect x='{x}' y='790' width='296' height='96' rx='24' fill='#f8fafc' stroke='#cbd5e1'/>
      <text x='{x+22}' y='822' font-family='Inter,Arial,sans-serif' font-size='14' font-weight='950' fill='#b91c1c'>WHEN {i+1}</text>
      <text x='{x+22}' y='851' font-family='Inter,Arial,sans-serif' font-size='14' fill='#475569'>{lines}</text>
    </g>""")

svg=f"""<svg xmlns='http://www.w3.org/2000/svg' width='1200' height='960' viewBox='0 0 1200 960'>
  <defs>
    <linearGradient id='hero' x1='0' x2='1' y1='0' y2='1'><stop stop-color='#111827'/><stop offset='.48' stop-color='#7f1d1d'/><stop offset='1' stop-color='#1e3a8a'/></linearGradient>
    <filter id='shadow' x='-12%' y='-20%' width='124%' height='150%'><feDropShadow dx='0' dy='14' stdDeviation='11' flood-color='#0f172a' flood-opacity='.13'/></filter>
  </defs>
  <rect width='1200' height='960' fill='#fff7f7'/>
  <rect x='42' y='38' width='1116' height='884' rx='44' fill='white' stroke='#fecaca' stroke-width='3'/>
  <rect x='74' y='72' width='1052' height='174' rx='36' fill='url(#hero)'/>
  <text x='108' y='120' font-family='Inter,Arial,sans-serif' font-size='18' font-weight='900' fill='#fecaca' letter-spacing='3'>AICLOUDSTRATEGIST · EVENING INFOGRAPHIC GUIDE</text>
  <text x='108' y='178' font-family='Inter,Arial,sans-serif' font-size='49' font-weight='950' fill='white'>{esc(TITLE)}</text>
  <text x='108' y='218' font-family='Inter,Arial,sans-serif' font-size='20' fill='#e0f2fe'>{esc(HOOK)}</text>
  <text x='72' y='274' font-family='Inter,Arial,sans-serif' font-size='23' font-weight='950' fill='#0f172a'>Six checks before AI-assisted text reaches a customer</text>
  {''.join(card_svg)}
  <text x='108' y='760' font-family='Inter,Arial,sans-serif' font-size='23' font-weight='950' fill='#0f172a'>Where to use the pause</text>
  {''.join(step_svg)}
</svg>"""
...
<style>body{{margin:0;background:#fff7f7;color:#152033;font-family:Inter,Segoe UI,Arial,sans-serif}}.wrap{{max-width:1080px;margin:auto;padding:32px 18px}}.hero{{background:linear-gradient(135deg,#111827,#7f1d1d,#1e3a8a);color:white;border-radius:30px;padding:36px}}.kicker{{color:#fecaca;text-transform:uppercase;letter-spacing:.12em;font-size:12px;font-weight:900}}h1{{font-size:43px;line-height:1.08;margin:14px 0}}.hook{{font-size:20px;line-height:1.45}}.card{{background:white;border:1px solid #fecaca;border-radius:24px;padding:24px;margin:22px 0;box-shadow:0 14px 35px rgba(15,23,42,.08)}}img{{max-width:100%;border-radius:24px;border:1px solid #fecaca}}li{{margin:12px 0;line-height:1.6}}.boundary{{background:#111827;color:#e5f0ff}}</style></head><body><main class='wrap'><section class='hero'><div class='kicker'>Evening publication · AI customer reply review</div><h1>{esc(TITLE)}</h1><p class='hook'>{esc(HOOK)}</p></section><section class='card'><img src='{SLUG}.png' alt='Infographic: {esc(TITLE)}'></section><section class='card'><h2>Six risk-pause checks</h2><ul>{checks_html}</ul></section><section class='card'><h2>Where to use it</h2><ol>{steps_html}</ol></section><section class='card boundary'><h2>Truth boundary</h2><p>{esc(BOUNDARY)}</p></section></main></body></html>"""
...
(info_dir / 'analysis.md').write_text(f"# Analysis — {TITLE}\n\n- Topic: AI-assisted customer-reply risk review.\n- Data type: educational checklist with six checks and three usage notes.\n- Layout: bento-grid risk dashboard.\n- Style: corporate Memphis with red/blue trust colors and card modules.\n- Audience: operators reviewing AI-assisted customer-facing messages.\n- Language: en.\n- Safety: educational only; no client, legal, compliance, medical, savings, ranking, or guaranteed claims.\n", encoding='utf-8')
(info_dir / 'structured-content.md').write_text(f"# Structured content — {TITLE}\n\n## Learning objective\nHelp teams pause AI-assisted customer-facing messages long enough to verify promises, evidence, private data, uncertainty, owner approval, and escalation needs.\n\n## Risk-pause checks\n{checks_md}\n## Usage steps\n{steps_md}\n## Boundary\n{BOUNDARY}\n", encoding='utf-8')
(info_dir / 'prompts' / 'infographic.md').write_text(f"Create a clean corporate Memphis bento-grid infographic titled '{TITLE}' using the six risk-pause checks and three usage steps from structured-content.md. Use red/blue trust colors, white cards, checkmark icons, clear hierarchy, and safe educational language. Aspect 16:9-ish wide worksheet.\n", encoding='utf-8')
# homepage latest card and link
home=REPO / 'index.html'
home_text=home.read_text(encoding='utf-8')
new_article=f'''          <article class="ea-evidence-item">
            <div class="ea-mini-art ea-architecture-art" aria-hidden="true"><span>Promise</span><i></i><span>Evidence</span><i></i><span>Escalate</span></div>
...
home_text=re.sub(r'/publications/\d{4}-\d{2}-\d{2}/[^"<>]+\.html">Review the [^<]+', f'/publications/{DATE}/{SLUG}.html">Review the AI customer promise risk pause', home_text, count=1)
...
if llms.exists():
    ...
    line=f'- [{TITLE}]({URL}) — safe educational AI customer-reply risk-pause checklist with infographic.\n'
    if URL not in txt:
