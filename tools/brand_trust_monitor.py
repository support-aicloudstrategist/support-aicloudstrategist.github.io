...
def scorecard(repo: dict, live: list[dict] | None = None) -> dict:
    ...
    parameters = [
        ...
        {"parameter": "High-intent landing pages", "percentage": 96 if all(pages[k] for k in ["healthcare", "cloud", "growth", "pricing", "paid_ad_plan"]) else 70, "status": "verified site-side", "evidence": "Commercial landing pages plus paid-ad conversion plan present."},
        ...
        {"parameter": "Search and visit monitoring", "percentage": 75 if pages["indexnow"] and pages["github_traffic"] else 55, "status": "blocked", "evidence": "Live technical monitoring, IndexNow submission, and GitHub repository traffic API capture exist, but no Search Console/Analytics/Cloudflare site visitor telemetry credentials are connected to Hermes."},
        ...
        {"parameter": "Backlinks/authority references", "percentage": 85 if target_link_citations >= 7 else 35, "status": "public citations verified; independent backlinks still building", "evidence": f"Verified public GitHub/Gist citations linking to AICS target pages: {target_link_citations}; independent third-party backlinks still cannot be fabricated."},
        {"parameter": "Brand-search demand creation", "percentage": 75 if pages["distribution_engine"] and pages["indexnow"] and verified_external_surfaces >= 10 else 45, "status": "blocked", "evidence": "Public brand surfaces/citations, IndexNow submission, and brand resource distribution exist; real brand-query evidence still needs Search Console/search-volume after distribution."},
